# Credit goes to https://github.com/TaoHuang2018/Neighbor2Neighbor 
...
class AugmentNoise(object):
    def __init__(self, style):
        if style.startswith('gauss'):
            ...
            print(self.params)
        elif style.startswith('gain'):  
            self.params = [float(p) for p in style.replace('gain', '', 1).split('_')]
            if len(self.params) == 1:
                self.style = "gain_fix"
            elif len(self.params) == 2:
                self.style = "gain_range"
    def add_train_noise(self, x):
        ...
        if self.style == "gauss_fix":
            ...
            noised = torch.poisson(lam * x, generator=get_generator()) / lam
            return noised
        elif self.style == "gain_fix":
            lam = self.params[0]
            lam = lam * torch.ones((shape[0], 1, 1, 1), device=x.device)
            noised = torch.poisson(x / lam, generator=get_generator()) * lam
            return noised
        elif self.style == "gain_range":
            min_lam, max_lam = self.params
            lam = torch.rand(size=(shape[0], 1, 1, 1), device=x.device) * (max_lam - min_lam) + min_lam
            noised = torch.poisson(x/lam, generator=get_generator()) * lam
            return noised
