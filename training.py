...
def nb2nb_aug_helper( img, noisy, dtype, config, experiment_cfg):
    ...
    for i in t:
        ...
        with torch.no_grad():
            ...
            noisy_psnr = psnr(out_full, img.detach().cpu(), max_val=1.0).item()
        ...
        # get l2 distance between 
        # new_model_params = [p.grad.data.detach().cpu().numpy() for p in model.parameters()]
        # ic(new_model_params[3])
        # for p in new_model_params:
        # ic((new_model_params[0] + new_model_params[1]).norm(2))
        # if model_params is not None:
        #     max_grad = max([(model_params[i] + new_model_params[i]).norm(2).item() for i in range(len(new_model_params))])
        #     grad_hist.append(max_grad)
            # print(max_grad)            
            # quit()
        # if model_params is not None:
        # ic(np.array(new_model_params).shape
        # ic(new_model_params)
        # ic(np.linalg.norm(np.array(model_params)+ np.array(new_model_params), 2). shape)
            # grad_hist.append()
    ...
    # print("A Decoder:{} B Encoder:{} C Decoder:{}".format(
    #     torch.norm(dict_checkpointA - model.apply_A.weight.data.clone().cpu()),
    #     torch.norm(dict_checkpointB - model.apply_B.weight.data.clone().cpu()),
    #     torch.norm(dict_checkpointC - model.apply_C.weight.data.clone().cpu()),
    #                                                                  ))
    # if 'mnist' in experiment_cfg['dataset'].keys():
    #     lpips_list= [-1.0]
    # else:
    #     loss_fn_alex = lpips.LPIPS(net='alex')
    #     with torch.no_grad():
    #             lpips_list =  [loss_fn_alex(out_avg.detach().cpu(), img.cpu()).item()]
    lpips_list = [0.0]
...
#barlow twins on the two down sampled images
# then see if passing in both the downsampled and the original noisy image for the 
def ours( img, noisy, dtype, config, experiment_cfg):
    model = build_model(config)
    model.type(dtype)
    ...
    if 'lr_sched' in experiment_cfg.keys():
        scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=800, gamma=.99)
    # swa_scheduler = SWALR(optimizer, swa_lr=0.05)
    ...
    for i in t:
        ...
        with torch.no_grad():
            ...
            noisy_denoised, sparse_rep = model(noisy_in)
            noisy_denoised = torch.clamp(noisy_denoised, 0.0, 1.0)
        ...
        #     noisy_output = model(noisy_sub1, torch.Tensor([50/255]).unsqueeze(1).unsqueeze(1).unsqueeze(1).type(dtype))
        # else:
        noisy_output, sparse_rep1 = model(noisy_sub1)
        _, sparse_rep2 = model(noisy_sub2)
        barlow_loss = model.barlow(sparse_rep1, sparse_rep2)
        # print("MODEL: {}".format(i))
        noisy_output = torch.clamp(noisy_output, 0.0, 1.0)
        # if H is not None:
        #         noisy_output = noisy_output[:,:, :H, :W]
        ...
        with torch.no_grad():
            ...
            #     out_full = swa_model(noisy).detach().cpu()
            # else:
            out_full = model(noisy)[0].detach().cpu()
            if H is not None:
                out_full = out_full[:,:, :H, :W]
        if out_avg is None:
            out_avg = out_full.detach().cpu()
        ...
        # with torch.no_grad():
            # lpips_score =  loss_fn_alex(out_avg.detach().cpu(), img.cpu()).item()
        t.set_description("PSNR:{:.5f} db | AVG:{:.5f} | | Loss: {:.5f}".format(clean_psnr, clean_psnr_avg, loss.item()))
        psnr_list.append(clean_psnr)
        loss_list.append(loss.item())
        # lpips_list.append(lpips_score)
        # scheduler.step(loss)
...
def dev_noisy_aug( img, noisy, dtype, config, experiment_cfg):
    model = build_model(config)
    model.type(dtype)
    ...
    noisy_in = noisy.clone()
    noisy_target = noisy.clone()
    # WITH CSCNET did nothing somehow
    # if 'rotate' in experiment_cfg.keys():
    #     noisy_in =  torch.cat((noisy, TF.rotate(noisy, 90), TF.rotate(noisy, 180), TF.rotate(noisy, 270)))
    ...
    for i in t:
        ...
        # if cfg['experiment_cfg']['loss'] == 'poisson':
        #     total_loss = pll(out, img_noisy_torch)
        net_input = noisy.clone() + (torch.rand_like(noisy.clone()).normal_() * reg_noise_std)
        net_input = torch.clamp(net_input, 0.0, 1.0)
        out = model(net_input)
    ...
    clean_psnr = psnr(out_avg, img.detach().cpu(), max_val=1.0)
    torch.save(model, '/home/cegrad/calta/sparse-dip/testmodel.pth')
    return out_avg, clean_psnr.item(), psnr_list, loss_list, lpips_list, []
